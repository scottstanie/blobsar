...
def calc_detection_stats(blobs_real, detected, min_iou=0.5, verbose=True):
    ...
    for cur_clob in detected:
        ...
        iou_frac = skblob.intersection_over_union(
    ...
    if 2 * len(true_detects) + len(false_positives) + len(misses) != len(
        blobs_real
    ) + len(detected):
...
def demo_ghost_blobs(
    num_blobs=10, min_iou=0.5, out=None, real_blobs=None, noise_sigma=0.5, max_ecc=0.4
):
    np.random.seed(1)
    finding_params = {
        "positive": True,
    ...
    detected, sigma_list = find_blobs(out, **finding_params)
...
def plot_blob_results(image, true_d=None, fp=None, misses=None, delete=False):
    ...
    if true_d is not None:
        fixed, ax = plot.plot_blobs(
            image=image, blobs=true_d, color="green", delete=delete
        )
    if fp is not None:
        fixed, ax = plot.plot_blobs(
            image=image, blobs=fp, color="black", ax=ax, delete=delete
        )
...
def make_edge(
    imsize, row=None, col=None, jump=1, max_val=2, min_val=0, noise_sigma=0.2
):
    """Create a discontinuity to check for edge detection/igoring"""
    rows, cols = imsize
    ramp = np.dot(
        np.ones((rows, 1)), np.linspace(1, 1 + max_val, cols).reshape((1, cols))
    )
    ramp = ramp - 1 + min_val
    if col is None:
        col = cols // 2
...
def plot_func(
    func=GAUSSIAN, shape=None, sigma=None, surface=None, ax=None, cmap=cm.coolwarm
):
    if surface is None:
        if shape is None:
            shape = (501, 501)
    ...
    ax.plot_surface(X, Y, surface, cmap=cmap, linewidth=0, antialiased=True)
    # plt.show()
    return ax
...
def demo_shape_index():
    ...
    a2 = axes[1, 0].imshow(
        feature.shape_index(bowl, sigma=1, mode="nearest"), vmin=-1, vmax=1
    )
    axes[1, 0].set_title("shape index:bowl")
    axes[1, 1].imshow(
        feature.shape_index(valley, sigma=sigma, mode="nearest"), vmin=-1, vmax=1
    )
    axes[1, 1].set_title("shape index:valley")
    axes[1, 2].imshow(
        feature.shape_index(gauss, sigma=sigma, mode="nearest"), vmin=-1, vmax=1
    )
...
def load_run(run_idx, data_path="."):
    ...
    run_arrays = {
        ...
        "miss_patches": [
            miss_arrs[key] for key in miss_arrs.keys() if key.startswith("arr")
        ],
...
def correct_run(run_idx=None, data_path="."):
    ...
    _, ax = plot.plot_blobs(image=image, blobs=true_d, color="green", delete=False)
    fp_fix, ax = plot.plot_blobs(image=image, blobs=fp, color="black", delete=True)
    # plt.show(block=True)

    # The misses we just want to get rid of if they are bad
    misses_fix, ax = plot.plot_blobs(
        image=image, blobs=misses, color="red", delete=True
    )
    plt.show(block=True)
...
def split_pos_neg(blobs):
    ...
    return np.array([b for b in blobs if b[3] > 0]), np.array(
        [b for b in blobs if b[3] < 0]
    )
...
def prune_nonzero_overlap(blobs, overlap=0.01, min_ratio=3):
    ...
    for bs in split_pos_neg(blobs):
        out = prune_annihilated_blobs(
            bs, min_ratio=min_ratio, max_overlap=overlap, keep_on="sigma"
        )
        # out = skblob.prune_overlap_blobs(bs, overlap, sigma_bins=1)
        if out.size:
...
def prune_annihilated_blobs(blobs, min_ratio=2, max_overlap=0.5, keep_on="amplitude"):
    ...
    for i, j in itertools.combinations(range(len(blobs)), 2):
        ...
        if sigma_ratio < min_ratio and skblob.blob_overlap(b1, b2) > max_overlap:
            # Keep
...
def make_stack2(N=501, max_amp=3, cmap="RdBu_r", ax=None, cbar_label="[cm]"):
    ...
    spacing = 80
    lr, lc = 345, 345
    b4 = make_gaussian(shape, 29, lr, lc)
    b4 += make_gaussian(shape, 29, spacing + lr, lc)
    b4 += make_gaussian(shape, 29, lr, spacing + lc)
    b4 += make_gaussian(shape, 29, spacing + lr, spacing + lc)
    ...
    if ax is None:
        ...
        fig = ax.get_figure()
    axim = ax.imshow(out, cmap=cmap, vmin=-vm, vmax=vm)
...
def make_stack3(N=501, max_amp=3, cmap="RdBu_r", ax=None, cbar_label="[cm]"):
    """Simplest composite of 2 blobs, one elliptical"""
    shape = (N, N)
    sigma1 = 50
    # b1 = make_gaussian(shape, 60, N // 3, 2 * N // 3, normalize=True)
    b1 = make_gaussian(shape, sigma1, N // 3, N // 3, normalize=True)
    b2 = make_gaussian_ellipse(shape, sigma=30, ecc=.6, theta=-45, row=345, col=345, normalize=True)
    print(b2.max(), b2.min(), b2.mean())
    print(b1.max(), b1.min(), b1.mean())
...
def igarss_fig(cmap="jet"):
    # out, fig = make_stack()
    out, fig = make_stack2()
    blobs, sigma_list = find_blobs(out, min_sigma=3, max_sigma=100, num_sigma=40)
    image_cube = skblob.create_gl_cube(out, sigma_list=sigma_list)

    # _, ax = plot.plot_blobs(image=out,
    # blobs=blob.find_edge_blobs(blobs, out.shape)[0],
    # ax=fig.gca(),
    # color='blue')

    plt.imshow(image_cube[:, :, 30], cmap=cmap, vmin=-1.4, vmax=1.3)
    plt.imshow(image_cube[:, :, 10], cmap=cmap, vmin=-1.4, vmax=1.3)
    return out, blobs, sigma_list, image_cube, fig


def igarss_fig2(out, blobs, sigma_list, image_cube, cmap="coolwarm"):
    fig = plt.figure(frameon=False)
    ax = fig.gca(projection="3d")
    ax.plot_surface(X, Y, f, cmap=cmap, linewidth=0, antialiased=True)
    plt.show()
